...
import { Pill, Plus, Calendar, User, Trash2, Edit3, X, Camera, Loader2, CheckCircle, AlertCircle, Printer } from "lucide-react";
import { formatDate } from "@/lib/utils";
import InteractionChecker from "./InteractionChecker";

function printMedicationList(medications: any[]) {
  ...
  const medsHtml = medications.map((med) => `
    <div class="med">
      ...
      <div class="badges">
        ${med.dosage ? `<span class="badge">${med.dosage}</span>` : ""}
        ${med.frequency ? `<span class="badge badge-green">${med.frequency}</span>` : ""}
      </div>
      <div class="details">
        ${med.prescribedFor ? `<div class="detail-row"><span class="detail-label">Condition:</span> ${med.prescribedFor}</div>` : ""}
        ${med.prescribedBy ? `<div class="detail-row"><span class="detail-label">Prescribed by:</span> ${med.prescribedBy}</div>` : ""}
        ${med.startDate ? `<div class="detail-row"><span class="detail-label">Started:</span> ${med.startDate}</div>` : ""}
        ${med.notes ? `<div class="detail-row notes"><span class="detail-label">Notes:</span> ${med.notes}</div>` : ""}
      </div>
    </div>`).join("");
...
<head>
  ...
  <style>
    ...
    .med { border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px 20px; margin-bottom: 14px; page-break-inside: avoid; }
    .med-name { font-size: 18px; font-weight: 700; color: #1a3a2a; margin-bottom: 6px; }
    .badges { display: flex; gap: 8px; flex-wrap: wrap; margin-bottom: 10px; }
    .badge { background: #f3f4f6; color: #374151; font-size: 12px; font-weight: 600; padding: 3px 12px; border-radius: 20px; border: 1px solid #e5e7eb; }
    .badge-green { background: #f0fdf4; color: #166534; border-color: #bbf7d0; }
    .details { display: grid; grid-template-columns: 1fr 1fr; gap: 5px 16px; }
    .detail-row { font-size: 13px; color: #374151; }
    .detail-row.notes { grid-column: span 2; }
    .detail-label { font-weight: 600; color: #6b7280; }
    .emergency { background: #fef2f2; border: 1px solid #fca5a5; border-radius: 8px; padding: 12px 16px; font-size: 13px; color: #991b1b; margin-top: 24px; font-weight: 600; }
    .disclaimer { background: #fef3c7; border: 1px solid #fcd34d; border-radius: 8px; padding: 12px 16px; font-size: 12px; color: #92400e; margin-top: 12px; }
    .footer { margin-top: 24px; text-align: center; color: #9ca3af; font-size: 11px; border-top: 1px solid #e5e7eb; padding-top: 12px; }
    .count { display: inline-block; background: #f0fdf4; color: #166534; border: 1px solid #bbf7d0; padding: 2px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; margin-left: 8px; }
  </style>
</head>
<body>
<div class="page">
  <h1>My Medication List <span class="count">${medications.length} medication${medications.length !== 1 ? "s" : ""}</span></h1>
  <p class="subtitle">Printed on ${today} · For use as a personal reference at medical appointments</p>

  ${medsHtml}

  <div class="emergency">⚠ In case of emergency or accidental overdose, call 911 or Poison Control: 1-800-222-1222</div>
  <div class="disclaimer"><strong>Medical Disclaimer:</strong> This medication list is for personal reference and informational purposes only. Always consult your prescribing physician or pharmacist before making any changes to your medications, dosages, or schedule. Never stop or change a medication without professional guidance.</div>
      ...
      <FadeIn className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {medications && medications.length > 0 && (
            <Button
              variant="outline"
              onClick={() => printMedicationList(medications)}

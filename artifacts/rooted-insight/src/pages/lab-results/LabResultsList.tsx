...
import { Activity, Plus, ChevronRight, AlertCircle, FileX } from "lucide-react";
...
export default function LabResultsList() {
  ...
  const sortedResults = results ? [...results].sort((a, b) => new Date(b.testDate).getTime() - new Date(a.testDate).getTime()) : [];
      ...
      {sortedResults.length === 0 ? (
        <FadeIn className="flex flex-col items-center justify-center p-12 text-center bg-white rounded-[2rem] border border-border border-dashed">
          ...
          <p className="text-muted-foreground max-w-md mb-8">
            Start tracking your health journey by manually entering your first lab result.
          </p>
          <Link href="/lab-results/add">
                  ...
                  <Card className="h-full cursor-pointer hover:shadow-lg hover:border-primary/30 group transition-all duration-300">
                    <CardContent className="p-6 flex flex-col h-full">
                      <div className="flex justify-between items-start mb-4">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            {formatDate(result.testDate)} {result.labName && `• ${result.labName}`}
                          </p>
                        </div>
                      ...
                      <div className="mt-auto pt-6 border-t border-border flex items-center gap-3">
                        ...
                        {result.analysis && (
                          <Badge variant="default" className="ml-auto bg-primary/10 text-primary border-transparent">Analysis Ready</Badge>
                        )}
                      </div>
                    </CardContent>
                  </Card>

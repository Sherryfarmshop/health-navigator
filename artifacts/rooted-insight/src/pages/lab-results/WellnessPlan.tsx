...
function buildPrintWellnessPlanHtml(plan: WellnessPlanData): string {
  const priorityColor = { high: "#dc2626", medium: "#d97706", low: "#16a34a" };
  const priorityLabel = { high: "High Priority", medium: "Medium Priority", low: "Monitoring" };

  const stepsHtml = [...plan.steps]
    .sort((a, b) => ({ high: 0, medium: 1, low: 2 }[a.priority] - ({ high: 0, medium: 1, low: 2 }[b.priority])))
    .map((step) => `
      <div style="margin-bottom:18px;padding:14px 18px;border-left:4px solid ${priorityColor[step.priority]};background:#fafafa;border-radius:0 10px 10px 0;page-break-inside:avoid;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;">
          <span style="background:${priorityColor[step.priority]}22;color:${priorityColor[step.priority]};padding:2px 10px;border-radius:20px;font-size:11px;font-weight:700;text-transform:uppercase;">${priorityLabel[step.priority]}</span>
          <span style="font-size:11px;color:#6b7280;text-transform:uppercase;letter-spacing:0.5px;">${step.category}</span>
        </div>
        <p style="margin:2px 0 4px;font-weight:700;font-size:15px;color:#111;">${step.marker}</p>
        <p style="margin:2px 0;font-size:12px;color:#6b7280;font-style:italic;">${step.finding}</p>
        <p style="margin:6px 0 3px;font-size:14px;color:#111;"><strong>Action:</strong> ${step.action}</p>
        ${step.actionDetail ? `<p style="margin:2px 0;font-size:12px;color:#4b5563;">${step.actionDetail}</p>` : ""}
        <p style="margin:6px 0 0;font-size:12px;color:#374151;"><strong>Start:</strong> ${step.timeline} &nbsp;|&nbsp; <strong>Retest in:</strong> ${step.retestIn}</p>
      </div>`)
    .join("");
  ...
  const questionsHtml = (plan.questionsForDoctor ?? [])
    .map((q, i) => `
      <div style="margin-bottom:14px;page-break-inside:avoid;">
        ...
        <div style="margin-top:6px;border-bottom:1px solid #d1d5db;height:20px;"></div>
...
<head>
  <meta charset="utf-8"/>
  <title>Wellness Action Plan — ${plan.testName}</title>
  <style>
    @media print { body { -webkit-print-color-adjust: exact; } @page { margin: 0.75in; } }
    body { font-family: Georgia, serif; color: #111; margin: 0; background: #fff; }
    .page { max-width: 760px; margin: 0 auto; padding: 48px 40px; }
    h1 { font-size: 26px; color: #1a3a2a; border-bottom: 2px solid #1a3a2a; padding-bottom: 10px; margin-bottom: 4px; }
    h2 { font-size: 17px; color: #1a3a2a; margin-top: 28px; margin-bottom: 12px; border-bottom: 1px solid #e5e7eb; padding-bottom: 6px; }
    .meta { color: #6b7280; font-size: 13px; margin-bottom: 24px; }
    .summary { background: #f0fdf4; border: 1px solid #86efac; border-radius: 8px; padding: 14px 16px; font-size: 14px; line-height: 1.7; margin-bottom: 8px; }
    .disclaimer { background: #fef3c7; border: 1px solid #fcd34d; border-radius: 8px; padding: 12px 16px; font-size: 12px; color: #92400e; margin-top: 28px; }
    .footer { margin-top: 24px; text-align: center; color: #9ca3af; font-size: 11px; border-top: 1px solid #e5e7eb; padding-top: 12px; }
  </style>
</head>
<body>
<div class="page">
  <h1>Wellness Action Plan</h1>
  <p class="meta"><strong>Test:</strong> ${plan.testName} &nbsp;|&nbsp; <strong>Date:</strong> ${plan.testDate} &nbsp;|&nbsp; <strong>Generated:</strong> ${plan.generatedAt}</p>
  ...
  ${plan.questionsForDoctor?.length ? `
  <h2>Questions to Ask Your Doctor</h2>
  ${questionsHtml}` : ""}

  <div class="disclaimer"><strong>⚠ Medical Disclaimer:</strong> This wellness plan is for educational and informational purposes only. It does not constitute medical advice, diagnosis, or treatment. Always consult a qualified healthcare provider before making changes to your health regimen, diet, supplements, or medications. Do not disregard professional medical advice or delay seeking it because of information presented here. In a medical emergency, call 911.</div>
  <div class="footer">Generated by Rooted Insight · For personal use only · Not a substitute for professional medical advice</div>
</div>
</body>
</html>`;
}

function buildPrintQuestionsHtml(plan: WellnessPlanData): string {
  const questionsHtml = (plan.questionsForDoctor ?? [])
    .map((q, i) => `
      <div style="margin-bottom:22px;page-break-inside:avoid;">
        <div style="padding:12px 16px;background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;font-size:14px;color:#1e3a5f;line-height:1.6;">
          <strong style="color:#1d4ed8;">${i + 1}.</strong> ${q}
        </div>
        <p style="font-size:11px;color:#9ca3af;margin:8px 0 2px;">Doctor's response / notes:</p>
        <div style="border-bottom:1px solid #d1d5db;height:22px;margin-bottom:4px;"></div>
        <div style="border-bottom:1px solid #d1d5db;height:22px;margin-bottom:4px;"></div>
        <div style="border-bottom:1px solid #d1d5db;height:22px;"></div>
      </div>`)
    .join("");
...
<head>
  <meta charset="utf-8"/>
  <title>Doctor Questions — ${plan.testName}</title>
  <style>
    @media print { body { -webkit-print-color-adjust: exact; } @page { margin: 0.75in; } }
    body { font-family: Georgia, serif; color: #111; margin: 0; background: #fff; }
    ...
    .intro { background: #f0fdf4; border: 1px solid #86efac; border-radius: 8px; padding: 12px 16px; font-size: 13px; line-height: 1.6; color: #374151; margin-bottom: 24px; }
    .disclaimer { background: #fef3c7; border: 1px solid #fcd34d; border-radius: 8px; padding: 10px 14px; font-size: 11px; color: #92400e; margin-top: 28px; }
    .footer { margin-top: 20px; text-align: center; color: #9ca3af; font-size: 10px; border-top: 1px solid #e5e7eb; padding-top: 12px; }
  </style>
</head>
<body>
<div class="page">
  <h1>Questions for My Doctor</h1>
  <p class="meta"><strong>Lab Test:</strong> ${plan.testName} &nbsp;|&nbsp; <strong>Date:</strong> ${plan.testDate} &nbsp;|&nbsp; <strong>Generated:</strong> ${plan.generatedAt}</p>
  <div class="intro">Bring this sheet to your appointment. Each question references your actual lab values. Use the lines below each question to record your doctor's response.</div>
  ${questionsHtml}
  <div class="disclaimer"><strong>⚠ Note:</strong> These questions were generated by an AI health education tool and are for discussion purposes only. They do not constitute medical advice. Your physician's guidance supersedes any AI-generated content.</div>
  <div class="footer">Generated by Rooted Insight · Bring this to your appointment · For discussion with your healthcare provider</div>
</div>
</body>
</html>`;
}
      ...
      <div className="flex items-center justify-between">
        ...
        {plan && (
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {/* Print Plan — prominent, always visible */}
            <Button
              variant="outline"
              size="sm"
              onClick={() => printHtml(buildPrintWellnessPlanHtml(plan))}
              className="gap-1.5 text-primary border-primary/40 hover:bg-primary/5"
            >
              <Printer className="w-4 h-4" /> Print Plan
            </Button>

            {/* Export dropdown — text file downloads */}
            <div className="relative" ref={exportRef}>
              ...
              {exportOpen && (
                <div className="absolute right-0 top-full mt-1 w-56 bg-white rounded-2xl shadow-xl border border-border z-50 overflow-hidden">
                  ...
                  >
                    <p className="font-semibold">Simple Version (.txt)</p>
                    <p className="text-xs text-muted-foreground">Download as plain text file</p>
                  </button>
                  <button
                    className="w-full text-left px-4 py-3 text-sm hover:bg-muted/50 transition-colors border-b border-border"
                    onClick={() => handleExport("checklist")}
                  >
                    <p className="font-semibold">Checklist Version (.txt)</p>
                    <p className="text-xs text-muted-foreground">Color-coded priority checklist file</p>
                  </button>
                  <button
                    className="w-full text-left px-4 py-3 text-sm hover:bg-muted/50 transition-colors"
                    onClick={() => handleExport("doctor")}
                  >
                    <p className="font-semibold text-primary">Full Doctor's Report</p>
                    <p className="text-xs text-muted-foreground">Lab values, steps + questions (print)</p>
                  </button>
                </div>
              )}
            </div>
              ...
              <Card className="border-blue-100 bg-blue-50/50 shadow-none">
                <CardContent className="p-6">
                  <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
                    <h3 className="font-display text-lg flex items-center gap-2 text-blue-900">
                      <Stethoscope className="w-5 h-5 text-blue-600" /> Questions to Ask Your Doctor
                    </h3>
                    <button
                      onClick={() => printHtml(buildPrintQuestionsHtml(plan))}
                      className="flex items-center gap-1.5 text-xs font-semibold text-blue-700 bg-blue-100 hover:bg-blue-200 border border-blue-200 px-3 py-1.5 rounded-lg transition-colors"
                    >
                      <Printer className="w-3.5 h-3.5" /> Print Questions
                    </button>
                  </div>
                  <p className="text-xs text-blue-600/70 mb-4 italic">The print version includes blank lines for recording your doctor's responses at the appointment.</p>
